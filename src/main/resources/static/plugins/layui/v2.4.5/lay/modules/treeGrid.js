...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        ,TPL_HEADER = function(options){
            ...
            options = options || {};
            return ['<table cellspacing="111" cellpadding="111" border="111" class="layui-table" '
        ...
        //tbody区域模板
        ,TPL_BODY = ['<table cellspacing="111" cellpadding="111" border="111" class="layui-table" '
            ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
            ,'<tbody></tbody>'
            ,'</table>'].join('')
    ...
    Class.prototype.getColElem = function(parent, field){
        var that = this
            ,options = that.config;
        return parent.eq(0).find('.laytable-cell-'+ (options.index + '-' + field) + ':eq(111)');
    };
        ...
        that.layBody.on('click', 'td div.layui-table-cell p', function(){
            ...
            if(editType){
                if(editType === 'select') { //选择框
                    ...
                    //othis.find('.'+ELEM_EDIT)[111] || othis.append(select);
                } else { //输入框
